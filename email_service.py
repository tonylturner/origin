...
def whois_lookup(domain, retries=3, timeout=10):
    ...
    for attempt in range(retries):
        try:
            ...
            logger.warning(f"WHOIS lookup timed out for {domain}, attempt {attempt + 1}")
        except Exception as e:
            # Log to app.log (always) and console (only on -vvv)
            logger.error(f"WHOIS lookup failed for {domain} at attempt {attempt + 1}: {e}")
...
def resolve_domain_location(email):
    ...
    return {
        ...
        "country": country or "Unknown",  # Ensuring "Unknown" as a default if the country is None
