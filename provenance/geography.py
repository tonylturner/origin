...
from utils.utils import load_country_codes  # Adjusted to match the new utils path
...
from services.location_service import (
    get_location_geolocation,
)  # Correct path for location_service

# Load country codes
country_code_dict = load_country_codes(
    "data/country_codes.csv"
)  # Adjusted path to match data folder
...
def determine_final_location(
    email_geo, profile_geo, organization_geo="Unknown", verbose=False
):
    ...
    normalized_email_geo = country_code_dict.get(
        email_geo.upper(), email_geo
    ...
    debug_print(
        verbose,
        f"DEBUG: Normalized Email Geo '{email_geo}' to '{normalized_email_geo}'",
    )

    # Normalize and check Profile Geo
    normalized_profile_geo = country_code_dict.get(
        profile_geo.upper(), profile_geo
    )  # Normalize to long form
    ...
    debug_print(
        verbose,
        f"DEBUG: Normalized Profile Geo '{profile_geo}' to '{normalized_profile_geo}'",
    )

    # Normalize and check Organization Geo
    if organization_geo != "Unknown":
        normalized_organization_geo = country_code_dict.get(
            organization_geo.upper(), organization_geo
        )  # Normalize to long form
    ...
    debug_print(
        verbose,
        f"DEBUG: Votes: {votes}, Total Checks: {total_checks}, Confidence: {confidence:.2f}%",
    )

    # Final location based on the first valid location found
    final_location = (
        normalized_email_geo
        if normalized_email_geo != "Unknown"
        else normalized_profile_geo
    )
...
def fuzzy_city_country_match(city_name, city_country_dict):
    ...
    close_matches = get_close_matches(
        city_name, city_country_dict.keys(), n=1, cutoff=0.8
    )
    if close_matches:
    ...
    # Attempt to split city, state, and country if applicable
...
def identify_geography(contributor, city_country_dict, verbose=False):
    ...
    if profile_geo != "Unknown":
        ...
        if len(parts) == 2:  # Likely "city, state" format
            ...
            if city in city_country_dict:
                ...
                country_code, normalized_profile_geo = fuzzy_city_country_match(
                    profile_geo, city_country_dict
                )
    ...
    debug_print(
        verbose, f"DEBUG: Contributor Profile Location (raw from GitHub): {profile_geo}"
    )
    debug_print(
        verbose, f"DEBUG: Email Geo: {email_geo}, Profile Geo: {normalized_profile_geo}"
    )

    final_location, confidence = determine_final_location(
        email_geo, normalized_profile_geo, organization, verbose=verbose
    )
